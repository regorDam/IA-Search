...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        for i in range(0,len(state[1])):
            if (not state[1][i] and not state[0] == self.corners[i]):
                return False
        else:
            return True
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0,len(problem.corners)):
        if state[1][i] == False:
            dist = util.manhattanDistance(state[0], problem.corners[i])
            if dist > tempDist:
                tempDist = dist

    return tempDist # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        total.append(mazeDistance(position,food,problem.startingGameState))
    if total:
        return max(total)
    return 0
